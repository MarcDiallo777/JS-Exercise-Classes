...
class Airplane {
 ...
 class Person{
  ...
  eat(someFood){
   ...
   if (stomach.length === 10){
     return "Ugh, my stomach is full!";
   }
  }
  
  poop(){
    for (let i = 0; stomach.length > i; ){
      stomach.pop;
    }
  }
 ...
 class Car{
  constructor(features){
    ...
    this.milesPerGallon = features.milesPerGallon;
    let tank = 0;
    let odometer = 0;
  }
  
  fill(money){
    for (let i = money; tank < i; ){
      tank++;
    }
  }
}
...
class Instructor extends Lambdasian{
  ...
  demo(subject){
    return `Hello class! Today we will learn about ` + subject;
  }
  
  grade(student, subject){
    return `${student.name} got a perfect score on ` + subject;
  }
}
  /*
    TASK 5
 ...
 class Student extends Lambdasian{
  constructor(traits){
    ...
    this.favClass = traits.favClass;
  }
  
  listSubjects(favClass){
    return "Loving " + favClass;
  }
  ...
  sprintChallenge(){
    return `${student.name} has completed the sprint challenge!`
  }
}
